...
namespace OrderManage
{
    public class Order : IComparable<Order>
    {
      ...
      //  string CommodityName;
        ...
        {
            Console.WriteLine("客户"+Client+"的订单已生成：");
            Console.WriteLine("订单号：" + OrderNumber);
        }
        public override bool Equals(object obj)
        {
            var o = obj as Order;
            if (o != null && OrderNumber == o.OrderNumber && Client == o.Client)
            {
                for (int j = 0; j < 8; j++)
                {
                    if (o.Amount[j] != Amount[j])
                        return false;
                }
                return true;
            }
        ...
        public override string ToString()
        {
            ...
            for(int k=0;k<8;k++)
            {
                result += $"{ls.AllCommodities[k]}:{Amount[k]}\r\n";
            }
            return result;
        }
        public int CompareTo(Order o)
        {
            if (o.OrderNumber > this.OrderNumber) return -1;
            else if (o.OrderNumber < this.OrderNumber) return 1;
            else return 0;
        }
   ...
   /* public class OrderDetails
    ...
    public class OrderService 
    {
        ...
        public void AddOrder()//添加订单
        {
            ...
            foreach (Order o in orders)
            {
                ...
                {
                    throw new Exception($"order-{NewOrder} is already existed!");
                }
            }
            orders.Add(NewOrder);
    ...
    class CommodityList
    {
        ...
        public override string ToString()
        {
            string result="";
            result += $"\r\nOur commodities: ";
            foreach(string j in AllCommodities )
            {
                result += $"{j} ";
            }
            result += $"\r\t";
